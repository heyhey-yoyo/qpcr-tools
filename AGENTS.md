...
视觉验收以工具主体正文 16px、操作标签不小于 12px 为基线；384 孔板紧凑注释可例外缩小。`YDchen Tools` 页眉结构与样式受保护，宽表和孔板只能在自身容器内横向滚动。
...
`_headers` 定义 Pages 静态资源的缓存再验证策略。
...
SVG 分隔线读取 plate-grid 的尺寸变量，与 96/384 孔板共享几何。图表用赤陶明度、纹理、逐柱基因文字及完整图例，超过六个基因仍可区分。存储失败保留内存态并提示；导出防止 CSV 公式注入，下载 URL 延迟回收。
