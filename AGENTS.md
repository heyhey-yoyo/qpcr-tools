...
- **刷新点板信息**（`loadPreset`）：按当前实验配置调用 `buildTemplate()` 生成模板，覆盖所有区块，起始位置重置为 A1。若孔板容量不足则弹窗拒绝
...
- **孔板比较组边界**：孔板预览虚线根据相邻孔位的 `clusterId` 绘制（如 N1T1 与 N2T2），仅分隔不同比较组；渲染层会在虚线下方绘制视觉空白缝，使虚线位于缝的中间，但不改变孔位编号或插入额外物理孔位
